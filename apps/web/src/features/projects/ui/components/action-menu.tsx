...
import {useDeleteProject} from "@/features/projects/lib/api";
...
export default function ActionMenu({rowData}: Props) {
    const [DeleteConfirmDialog, confirmDelete] = useConfirm("Silme Onayı", "Bu projeyi silmek istediğinize emin misiniz?", "Sil")

    const deleteProject = useDeleteProject(rowData.id)
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent>
                    ...
                    <DropdownMenuSeparator/>
                    <DropdownMenuItem>
                        <Link to={"/projects/edit-project/$id"} params={{
                            id: rowData.id
